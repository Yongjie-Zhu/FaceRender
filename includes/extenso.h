 ...
 *
 * Licensed under the MIT Licenese.
 *
 * @date:   Thu Jun 25 18:02:13 2020
 *
 * @brief:
 *
 *
...
public:
    ...
    void SyncNoOp() { next_update_time_ = std::chrono::system_clock::now(); }
...
inline void Print(std::string format) { std::cout << format; }
...
constexpr std::array<char, N1 + N2 - 2> __ConcatImpl(const char (&a)[N1], const char (&b)[N2],
...
template <std::size_t N1, std::size_t N2, std::size_t... Idxs1, std::size_t... Idxs2>
constexpr std::array<char, N1 + N2 - 1> __ConcatImpl(const char (&a)[N1], const std::array<char, N2>(&b),
                                                     std::index_sequence<Idxs1...>,
                                                     std::index_sequence<Idxs2...>)
{
    return {a[Idxs1]..., b[Idxs2]...};
...
struct AABB
{
    ...
    AABB(const std::vector<vec3>& ps) : AABB()
    {
        for (const auto& p : ps)
            Extend(p);
    }
    ...
    {
        return p.x <= sup.x && p.y <= sup.y && p.z <= sup.z && p.x >= inf.x && p.y >= inf.y && p.z >= inf.z;
    }

    vec3 Center() const { return (inf + sup) / 2.f; }

    vec3 Lengths() const { return sup - inf; }
};
...
inline std::string StringView(const mat3& m)
{
    return "((" + std::to_string(m[0][0]) + "," + std::to_string(m[1][0]) + "," + std::to_string(m[2][0]) +
           ...
           std::to_string(m[2][1]) + ")\n" + " (" + std::to_string(m[0][2]) + "," + std::to_string(m[1][2]) +
           "," + std::to_string(m[2][2]) + "))\n";
}
...
inline std::string StringView(const mat4& m)
{
    return "((" + std::to_string(m[0][0]) + "," + std::to_string(m[1][0]) + "," + std::to_string(m[2][0]) +
           "," + std::to_string(m[3][0]) + ")\n" + " (" + std::to_string(m[0][1]) + "," +
           std::to_string(m[1][1]) + "," + std::to_string(m[2][1]) + "," + std::to_string(m[3][1]) + ")\n" +
           " (" + std::to_string(m[0][2]) + "," + std::to_string(m[1][2]) + "," + std::to_string(m[2][2]) +
           "," + std::to_string(m[3][2]) + ")\n" + " (" + std::to_string(m[0][3]) + "," +
           std::to_string(m[1][3]) + "," + std::to_string(m[2][3]) + "," + std::to_string(m[3][3]) + "))";
}

inline std::string StringView(const quat& q)
{
    return std::string("(") + std::to_string(q.w) + "," + std::to_string(q.x) + "," + std::to_string(q.y) +
           "," + std::to_string(q.z) + ")";
}
...
{
public:
    ArcballCamera() : rot_{1.f, 0.f, 0.f, 0.f}, is_tracking_{false}, rot_prev_{1.f, 0.f, 0.f, 0.f} {}
    ...
    mat4 Proj() { return Perspective(FoVy_, aspect_, Near(), Far()); }
...
private:
    bool ISect(float mousex, float mousey, vec3* nhit) const
    {
        ...
        auto view = LookAt(unitpos_, {0.f, 0.f, 0.f}, vec3{.0f, 1.f, .0f});

        auto tmp11 = UnProject(winpos, view, Perspective(FoVy_, aspect_, Near(), Far()), view_port);
...
public:
    WanderCamera()
        : rot_{1.f, 0.f, 0.f, 0.f}, is_tracking_{false}, rot_prev_{1.f, 0.f, 0.f, 0.f}, up_{0.f, 1.f, 0.f}
    {
    }
    ...
    void Forward(float dist) { pos_ += (rot_ * ndir_) * dist; }

    int Width() const { return width_; }
    int Height() const { return height_; }
    vec3 Pos() { return pos_; }

    mat4 Proj() const { return Perspective(FoVy_, aspect_, Near(), Far()); }
...
private:
    bool ISect(float mousex, float mousey, vec3* nhit) const
    {
        ...
        auto tmp11 = UnProject(winpos, view, Proj(), view_port);
